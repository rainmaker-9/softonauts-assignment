  ...
  $.widget("ui.scrollTabs", $.ui.tabs, {
    ...
    options: {
      scrollOptions: {
        closable: true,
        customGotoFirstHTML:
          '<button class="stNavFirstArrow ui-state-active" title="First">' +
          '<span class="ui-icon ui-icon-seek-first">First tab</span></button>',
        customMoveNextHTML:
          '<button class="stNavNextArrow ui-state-active" title="Next">' +
          '<i class="fas fa-angle-double-right"></i></span></button>',
        customGoToLastHTML:
          '<button class="stNavLastArrow ui-state-active" title="Last">' +
          '<span class="ui-icon ui-icon-seek-end">Last tab</span></button>',
        customMovePreviousHTML:
          '<button class="stNavPrevArrow ui-state-active" title="Previous">' +
          '<i class="fas fa-angle-double-left"></i></span></button>',
        easing: "swing",
        enableDebug: false,
    ...
    _findScrollbarWidth: function () {
      ...
      if (this.sbarWidth === null) {
        ...
        parent = $(
          '<div class="__sb-test" style="width:50px;height:50px;overflow:auto;">' +
            "<div/></div>"
        ).appendTo("body");
    ...
    _debounceEvent: function (dbFunc) {
      return this.debounceEnabled
        ? $.debounce(this.eventDelay, dbFunc)
        : dbFunc;
    },
    /**
    ...
    _throttleEvent: function (dbFunc) {
      return this.debounceEnabled
        ? $.throttle(this.eventDelay, dbFunc)
        : dbFunc;
    },
      ...
      if ($.isFunction($.fn.mousewheel)) {
        ...
        this._on(this.$scrollDiv, {
          mousewheel: function (event) {
            ...
            self_1._scrollWithoutSelection(
              _this.navigateOptions.mouseScroll,
    ...
    _setupNavControls: function () {
      ...
      this.$leftArrowWrapper = $(
        this.options.scrollOptions.leftArrowWrapperHTML
      );
      this.$rightArrowWrapper = $(
        this.options.scrollOptions.rightArrowWrapperHTML
      );
    ...
    _showNavsIfNeeded: function () {
      ...
      var showRight = !(
        Math.abs(
          this.$scrollDiv[0].scrollWidth -
            this.$scrollDiv.scrollLeft() -
            this.$scrollDiv.outerWidth()
        ) < 1
      );
      ...
      showLeft
        ? this.$leftArrowWrapper.addClass("stNavVisible")
        : this.$leftArrowWrapper.removeClass("stNavVisible");
      showRight
        ? this.$rightArrowWrapper.addClass("stNavVisible")
    ...
    _getScrollDeltaValue: function ($tab) {
      ...
      if (leftPosition.left < currentScroll + arrowsWidth) {
        ...
        leftPosition.left + width + arrowsWidth >
    ...
    _moveToNextTab: function (e) {
      ...
      this._activate(
        this._findNextTab(Math.max(0, this.options.active + 1), true)
      );
    },
    /**
    ...
    _moveToPrevTab: function (e) {
      ...
      this._activate(
        this._findNextTab(Math.max(0, this.options.active - 1), false)
      );
    },
    /**
    ...
    addTab: function (header, panelContent) {
      ...
      var tab = $(
        '<li><a href="#' + newId + '" role="tab">' + header + "</a></li>"
      );
    ...
    _destroy: function () {
      ...
      this.element
        .removeClass(this.options.wrapperCssClass)
